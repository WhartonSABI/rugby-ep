...
\begin{abstract}
Following a penalty in Rugby Union, teams typically choose between attempting a kick at goal or kicking to touch to pursue a try. We develop an \emph{Expected Points} (EP) framework that quantifies the value of each option as a function of both field location and game context. Using phase-level data and observed penalty kicks, we construct two surfaces: (i) the expected points of a possession beginning with a lineout, and (ii) the expected points of taking a kick at goal. We then compare these surfaces to produce decision maps that indicate where kicking for goal or kicking to touch maximizes expected points, and we analyze how the boundary shifts with game context and the expected meters gained to touch. This study offers, to our knowledge, the first comprehensive EP-based assessment of penalty strategy in rugby union which can be refined in the future with richer tracking data or extended to win-probability estimation.
\end{abstract}

\section{Introduction}

Rugby union presents a recurring strategic decision immediately after a penalty with four possible ways to restart play: going for an immediate three points via a kick at goal or pursuing a potential seven-point try by either a scrum, a tap-and-go (both at the point of the penalty) or a kick to touch where the attacking team restarts play with a lineout. Except when right on the opposition goal-line, a lineout is perceived to be a superior choice than a tap-and-go or a scrum since the attacking team can safely advance the ball further down the field, with the lineout occurring where the ball crosses the touch line following the kick. This means that, once within distance to make a penalty kick, $\approx 65\text{m}$ from the opponent try line, the decision following a penalty is routinely between taking a lineout or kicking at goal.

While the choice is often straightforward near the opposition's corner, given a certain point difference, or late in a match, many game states are ambiguous and governed by convention, perceived kicker ability, and the expected quality of a team’s lineout. In contrast, American Football has long relied on \emph{Expected Points} (EP), \emph{Expected Points Added} (EPA), and \emph{win-probability} (WP) frameworks to guide the analogous fourth-down decision, translating field position and context into expected returns and actionable recommendations \citep{Romer2006,Burke2009,YurkoVenturaHorowitz2018}. In that setting, WP and EP estimators are typically built using statistical machine learning models (e.g., Random Forests and gradient boosting), as is done in \cite{LockNettleton2014} and \cite{Baldwin2021}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\linewidth]{zones.png}
...
Our kicking data was generously provided by Ken Quarrie, including international penalty observations from \cite{QuarrieHopkins2015} as well as kicks from Super Rugby, and NPC competition. The kicks span the years 2000--2012, with a total of 13,338 observations.

The data consists of exact $x, y$ locations of the penalties, allowing the calculation of the angle between the kick and the middle of the posts.

\begin{table}[h]
\centering
\begin{tabular}{ccccc}
\hline
\textbf{x} & \textbf{y} & \textbf{make} & \textbf{Distance} & \textbf{PlayerID} \\
\hline
39.5 & 40.5 & 1 & 41 & PL3QSQCQ30 \\
35.5 & 25.0 & 1 & 25 & PL3QSQCQ30 \\
8.2  & 39.7 & 0 & 48 & PL3QSQCQ30 \\
64.2 & 23.7 & 0 & 38 & PL3QSQCQ30 \\
27.3 & 42.8 & 1 & 44 & PL3QSQCQ30 \\
\hline
\end{tabular}
\caption{Sample of the penalty kick dataset. $x$ shows the distance from the left-hand touch line and $y$ is the distance from the opposition try line (metres), 
\texttt{make} indicates whether the kick was successful (1) or missed (0), 
\texttt{Distance} is the distance from the posts (metres), and \texttt{PlayerID} 
is the anonymised player identifier.}
\label{tab:kick_data}
\end{table}
...
\]
where $EP_{\text{kick}}(x, y)$ is the expected points from attempting a kick at goal from $(x, y)$, and $EP_{\text{lineout}}(x_{\text{LO}})$ is the expected points from a lineout at location $x_{\text{LO}}$ on the sideline. Here $x_{\text{LO}}$ denotes the resulting lineout $x$-coordinate after kicking to touch; we make this translation rule explicit in Section~\ref{sec:adjustment}.

The remainder of this section describes how we estimate $EP_{\text{lineout}}$ and $EP_{\text{kick}}$.


\subsection{Expected Points of a Lineout}
\label{sec:lineout}
To value a lineout, we model the probability of each \emph{next} scoring outcome using a multinomial logistic regression, based on possession sequences that begin with a lineout in a given field zone. Let $\mathcal{C}$ denote the set of observed next-score outcomes in the sample (from the perspective of the team in possession), which in our data is $\{-7, -5, -3, 0, 3, 5, 7\}$. For each outcome $c \in \mathcal{C}$, the model estimates
\[
\log \frac{P(Y = c \mid \mathbf{X})}{P(Y = c^* \mid \mathbf{X})}
  = \alpha_c + \beta_{c,1} \cdot \text{meter\_line\_factor}
  + \beta_{c,2} \cdot \text{Card\_Diff}
  + \beta_{c,3} \cdot \text{WinPct\_Diff},
\]
where $c^*$ is the reference outcome category and the coefficients $\{\alpha_c, \beta_{c,1}, \beta_{c,2}, \beta_{c,3}\}$ are estimated via maximum likelihood. The field position predictor \texttt{meter\_line} enters the model as a factor, allowing a flexible, non-linear relationship between field zone and scoring probability. Expected points are then recovered as a weighted sum over outcomes,
\[
EP_{\text{lineout}}(\mathbf{x}) = \sum_{c \,\in\, \mathcal{C}} c \cdot \hat{P}(Y = c \mid \mathbf{X}),
\]
where $\hat{P}(Y = c \mid \mathbf{X})$ is the fitted probability from the multinomial model.

We use a multinomial model (rather than a single linear regression on points) because the response is inherently a discrete state variable, and the decision problem depends on the full conditional outcome distribution, not only its mean. In particular, two game states can share similar mean points but imply very different risk profiles if their mass is distributed differently across no-score, penalty, and try outcomes. Modeling $P(Y=c\mid\mathbf{X})$ directly preserves this structure, after which EP is a transparent linear functional of estimated state probabilities. This follows the EP modeling logic used in football analytics, where next-score state probabilities are estimated first and then mapped into expected points \citep{YurkoVenturaHorowitz2018}.

This specification captures downstream contingencies observed in matches: losing one's own lineout, turnovers in subsequent phases, or the opponent scoring following a turnover. The resulting quantity represents the \emph{expected next-score value} of having a lineout at that field position, given competition-level play and the current game context. In the decision maps below, we evaluate this surface at the \texttt{meter\_line\_factor} level implied by each lineout location $x_{\text{LO}}$.
...
Consider a group of $n$ consecutive lineouts sharing the same \texttt{run\_id}. Treating all $n$ as independent observations would over-represent that specific game situation in the regression. Each lineout in the group shares the same next-score outcome (here 7 points for the team in possession, as seen in Table~\ref{tab:example-data}), yet in reality only one scoring event occurred. Including all $n$ lineouts as separate observations would therefore credit that single score $n$ times, distorting the regression toward outcomes that happened to follow long possession sequences.


To prevent this, we randomly retain a single representative observation from each group and discard the remaining $n-1$. Let $Y_1, \dots, Y_n$ denote the $n$ lineouts in such a group. The retained observation $\tilde{Y}$ is drawn uniformly:
\[
...
\subsubsection{Multinomial Regression Summary}

The lineout model treats the next scoring event as a categorical random variable with outcomes $\{-7, -5, -3, 0, 3, 5, 7\}$ points, and converts fitted class probabilities into EP through the probability-weighted sum defined above. 


\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{ep_by_meter_line.png}
    \caption{Average expected points of the next score given lineout position}
    \label{fig:ep-lineout}
\end{figure}

\subsubsection{EP Uncertainty Quantification using Bootstrap}

To quantify uncertainty in the expected points estimates, we used a percentile 
bootstrap with $B = 2000$ replicates. In each replicate, matches were resampled 
with replacement, preserving the match as the natural unit of variation. Within 
each resampled match, phases sharing the same \texttt{run\_id} --- indicating 
they belong to the same continuous passage of play --- had a single observation 
sampled at random, avoiding pseudo-replication from repeated measures within a 
run. The multinomial model was refitted on each bootstrap sample and used to 
generate predicted probabilities across all metre-line positions, from which 
expected points were derived. $95\%$ confidence intervals were then obtained 
from the 2.5th and 97.5th percentiles of the resulting bootstrap distribution.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{multinomial_bootstrap.png}
    \caption{Uncertainty quantification using bootstrap ($95\%$ Confidence intervals)}
    \label{fig:ep-lineout}
\end{figure}

Unsurprisingly, since the lineouts show teams with possession of the ball, we have large uncertainty due to a low number of lineout observations near a team's own try line. This isn't worrying to us since kicking for a goal isn't feasible from that distance anyway, we are only concerned about penalty decisions within roughly $65m$ from the other teams try line.
...
\subsection{Expected Points of a Kick}

\subsection{Estimating Kick Success Probability}

The data consists of exact $x, y$ locations of the penalties, allowing the calculation 
of the angle between the kick and the posts:
\[
\theta = \arctan\left(\frac{|x - 35|}{y}\right) \cdot \frac{180}{\pi}
\]
where $x$ is the distance from the left touchline, $y$ is the distance from the 
opposition try line, and 35 is the midpoint of the pitch width (in metres), so 
$|x - 35|$ is the horizontal distance from the centre of the posts. 
...
\begin{tikzpicture}[scale=1.2]
  ...
  % try line (horizontal)
  \draw[thick] (0, 0) -- (6, 0) node[right] {try line};

  % posts (at x=35, i.e. centre of pitch)
  \filldraw[black] (3, 0) circle (2pt) node[below] {posts $(35, 0)$};

  % kick location (to the LEFT of posts)
  \filldraw[black] (4.8, 3) circle (2pt) node[right] {kick $(x, y)$};

  % vertical line from posts upward (dashed)
  \draw[dashed] (3, 0) -- (3, 3.4);

  % line from posts to kick location
  \draw[thick] (3, 0) -- (4.8, 3);

  % angle arc
  \draw[->] (3, 0.8) arc (90:58:0.8) node[midway, above right] {$\theta$};

  % horizontal distance label
  \draw[<->] (3, 3.3) -- (4.8, 3.3) node[midway, above] {$|x - 35|$};

  % vertical distance label
  \draw[<->] (2.8, 0) -- (2.8, 3) node[midway, left] {$y$};

  % line from posts to kick location
  \draw[thick] (3, 0) -- (4.8, 3) node[midway, right] {distance};

\end{tikzpicture}
\caption{Illustration of the angle $\theta$ calculated for each penalty kick, 
where $y$ is the distance from the try line and $|x - 35|$ is the horizontal 
distance from the centre of the posts.}
\label{fig:angle}
\end{figure}

We use a simple 
logistic model with the distance from the posts and the angle as the only two predictors:
\[
\log\left(\frac{p_i}{1 - p_i}\right) = \beta_0 + \beta_1 \cdot \text{distance}_i + \beta_2 \cdot \theta_i
\]
where $p_i$ is the probability that kick $i$ is successful.


Estimated kick success probabilities, with dashed lines at probabilities of $(0.2, 0.4, 0.6, 0.8)$, are shown in Figure~\ref{fig:kick-success-prob}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{kick_prob_plot.png}
    \caption{Estimated probability of kick success by position on the pitch. Note that the center of the goal posts are at a lateral position 35\,m. Kick probabilities begin at 5\,m from the goal line}
...
5m--22m (own)   & 21 & -1.00 \\
22m--10m (own)  & 89 &  1.10 \\
10m--Half (own) & 45 & -0.44 \\
Half--10m (opp) & 22 &  1.23 \\
10m--22m (opp)  & 7  &  4.29 \\
\midrule
\textbf{Overall Average} & 198 & 0.60 \\
\bottomrule
\end{tabular}
\end{table}
...
+ \big(1 - \P_{\text{make}}(d, \theta)\big) \cdot EP_{\text{miss}}(d, \theta)
\]
to obtain the expected points of a kick at goal from that location. 

\subsection{Uncertainty Quantification of Kick}

There are two sources of uncertainty, firstly the uncertainty in the true success probability and secondly the uncertainty in the value of a missed kick. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{missed_bootstrap.png}
    \caption{Bootstrap of Points After a Missed Kick}
    \label{fig:kick-ep}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{cross_section_plot.png}
    \caption{Confidence Interval of kick Probability while changing distance from try line}
    \label{fig:kick-ep}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{cross_section_angle_plot.png}
    \caption{Confidence Interval of kick Probability while changing lateral position of Kick}
    \label{fig:kick-ep}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{delta_plot.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\linewidth]{baseline_with_marker.png}
...
\midrule
\textbf{Value} & 2.54 & 2.45 & 0.09 \\
\bottomrule
\end{tabular}
\end{table}

Here $\Delta EP = 0.09$ in favour of the lineout, implying that by electing to kick at goal in this situation, South Africa, in expectation, forfeited approximately 0.09 points relative to the lineout option.
...
\hline
NZ & 1.56 & 1.31 & kick     & lineout  & 0.25 \\
NZ & 1.90 & 1.47 & lineout  & lineout  & 0.00 \\
SA & 1.42 & 1.35 & lineout  & lineout  & 0.00 \\
SA & 2.54 & 2.08 & kick     & lineout  & 0.46 \\
SA & 1.63 & 1.10 & kick     & lineout  & 0.53 \\
SA & 1.38 & 0.97 & lineout  & lineout  & 0.00 \\
NZ & 2.68 & 2.38 & kick     & lineout  & 0.29 \\
SA & 1.74 & 1.61 & lineout  & lineout  & 0.00 \\
NZ & 3.35 & 2.08 & lineout  & lineout  & 0.00 \\
NZ & 3.35 & 2.33 & lineout  & lineout  & 0.00 \\
SA & 2.42 & 2.04 & lineout  & lineout  & 0.00 \\
SA & 1.03 & 0.64 & lineout  & lineout  & 0.00 \\
SA & 3.16 & 2.76 & lineout  & lineout  & 0.00 \\
\hline
\end{tabular}
\end{table}
...
\hline
Total regret $R$ & 1.54 \\
Proportion of Optimal Decisions & 0.69 \\
\hline
\end{tabular}
\end{table}

As summarized in Table~\ref{tab:summary_metrics}, the model’s optimal EP decision was made 69\% of the time, leading to a total regret of 1.54 points in expectation between both teams. While the proportion of ``incorrect'' decisions seems relatively high, the expected points lost from sub-optimal decision making is modest. For context, the game ended 24--17 to New Zealand, so 1.54 points would not have swung the outcome. Given that these international teams are at the pinnacle of the sport, the observed level of inefficiency is perhaps unsurprising: most decisions are near the indifference frontier, where the cost of making the ``wrong'' choice is relatively small.
...
All code used to process the data, fit the models, and generate the figures and tables in this article is available on \href{https://github.com/WhartonSABI/rugby-ep}{GitHub}.
